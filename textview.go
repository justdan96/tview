...
func (t *TextView) GetTagData() (colors []*textViewIndex, regions []*textViewRegion) {
  colors, regions = t.index, t.regionInfos
  return
}
...
func (t *TextView) GetStyler() Styler {
  return t.styler
}
func (t *TextView) SetStyler(s Styler) *TextView {
	// if t.styler != s {
		t.index = nil
	// }
	t.styler =s 
	...
	for bufferIndex, str := range t.buffer {
		...
		for _, splitLine := range splitLines {
			...
			for {
				...
				case 0:
					// Process color tags.
					foregroundColor, backgroundColor, attributes = styleFromTag(foregroundColor, backgroundColor, attributes, colorTags[colorPos], t.styler)
					colorPos++
				case 1:
					// Process region tags.
